...
MySQL - 10.1.25-MariaDB : Database - waiter
*********************************************************************
*/
...
CREATE TABLE `cantones` (
  ...
  KEY `fk_cantones_provincia` (`id_provincia`),
  CONSTRAINT `fk_cantones_provincia` FOREIGN KEY (`id_provincia`) REFERENCES `provincias` (`id_provincia`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
insert  into `categoria`(`id`,`id_restaurante`,`nombre`) values 
(1,1,'Entradas'),
(2,1,'Platos Fuertes'),
(3,1,'bebidas');
...
CREATE TABLE `ciudad` (
  ...
  CONSTRAINT `FK_4786469191104EC2` FOREIGN KEY (`ubicacionpaisid`) REFERENCES `pais` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
CREATE TABLE `distrito` (
  ...
  KEY `pk_distrito_canton` (`id_canton`),
  CONSTRAINT `fk_distrito_canton` FOREIGN KEY (`id_canton`) REFERENCES `cantones` (`id_canton`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
/*Data for the table `payment_plans` */
...
CREATE TABLE `pedido` (
  ...
  CONSTRAINT `fk_id_usuario_pedido` FOREIGN KEY (`id_usuario`) REFERENCES `usuarios` (`id_usuario`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
(1,1,'1234','mini lasagna con ensalada','Lasagna con carne acompanada de ensalada cesar',3650,''),
(2,2,'12345','minicrepa de pollo con ensalada y papas','crepa de tortilla de harina con papas a la francesa y ensalada ',4350,''),
(3,2,'12455','medio lapiz integral','emparedado de pan integral ',2600,''),
(4,3,'12','capuccino frio',NULL,2250,'');
...
CREATE TABLE `restaurante` (
  ...
  `direccion` text,
  PRIMARY KEY (`id`),
  KEY `fk_id_gerente` (`gerente`),
  KEY `fk_id_distrito_r` (`id_distrito`),
  CONSTRAINT `fk_id_distrito_r` FOREIGN KEY (`id_distrito`) REFERENCES `distrito` (`id_distrito`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_id_gerente` FOREIGN KEY (`gerente`) REFERENCES `usuarios` (`id_usuario`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8;
...
/*Data for the table `user_group_users` */
...
CREATE TABLE `usuarios` (
  ...
  CONSTRAINT `fk_id_ciudad` FOREIGN KEY (`id_ciudad`) REFERENCES `ciudad` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8;
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_buscar_usuario`(IN email VARCHAR(50))
BEGIN
	if email = 'NULL' then
	 set email= NULL;
	end If;
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_comprobar_login`(in ucorreo varchar(30), in uclave VARCHAR(88))
BEGIN
	SELECT
	COUNT(*) AS login,
	u.id_roles
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_crear_usuario`(IN ced CHAR(15),IN nom VARCHAR(30),IN ape VARCHAR(30),in tel char(12), in email varchar(30), IN users VARCHAR(20), IN pass VARCHAR(88),IN rol TINYINT(2))
BEGIN
