...
class Login extends CI_Controller 
{
    	...
    	if($this->settings->info->activate_account) 
    	{
    		if(!$r->active) {
    			$this->template->error(lang("error_72") . "<a href='".
    				site_url("register/send_activation_code/" . $r->id_usuario . "/" .
    				 urlencode($r->email)).
	...
	public function google_login() 
	{
		...
		if ($client->getAccessToken()) {
		    ...
		    if ($user->num_rows() == 0) {
		    	// Create
		    	$this->register_model->register_google_user($provider, 
				$oauth_id, $name, $token);
		    } else {
		    	$this->login_model
	...
	public function facebook_login_pro() 
	{
		...
		if(isset($accessToken)) {
			...
			if ($user->num_rows() == 0) {
			// New user time
			$this->register_model->register_facebook_user($provider, 
				$oauth_id, $name, $token);
			} else {
				// login
				$this->login_model
	...
	public function twitter_login_pro() 
	{
		 ...
		 && !empty($_SESSION['oauth_token_secret']))
		{
			...
			if ($user->num_rows() == 0) {
				// Create User
				$userid = $this->register_model->register_twitter_user(
	...
	{	
		...
		$token = rand(10000000,100000000000000000) . "HUFI9e9dvcwjecw8392klle@O(*388*&&£^^$$$";

		$token = sha1(md5($token));

		$this->login_model->resetPW($user->id_usuario, $token);
		...
		$email_template->message = $this->common->replace_keywords(array(
			...
			"[SITE_URL]" => site_url(),
			"[EMAIL_LINK]" => 
				site_url("login/resetpw/" . $token . "/" . $user->id_usuario),
