...
class Register_Model extends CI_Model 
{
	...
	{
		$s=$this->db->where("email", $email)->get("usuarios");
		if ($s->num_rows() > 0) {
			return false;
		} else {
			return true;
	...
	{
		$s=$this->db->where("username", $username)->get("usuarios");
		if($s->num_rows() > 0) {
			return false;
		} else {
			return true;
	...
	public function add_username($userid, $username) 
	{
		...
		->update("usuarios", array("username" => $username));
	}
	...
	{
		$this->db->where("id_usuario", $userid)->update("usuarios", array("username" => $username, "email" => $email));
	}
