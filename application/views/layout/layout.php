...
<html lang="en">
   <head>
     ...
     <!-- Latest compiled and minified CSS -->
      ...
      <link href="http://prologin.patchesoft.com/styles/elements.css" rel="stylesheet" type="text/css">
         ...
         <div class="container-fluid">
            <div class="navbar-header">
               ...
               <a class="navbar-brand" href="<?php echo site_url() ?>" title="<?php echo $this->settings->info->site_name ?>">
                    <img src="<?php echo base_url() ?><?php echo $this->settings->info->upload_path_relative ?>/<?php echo $this->settings->info->site_logo ?>" width="50%" style="margin-top: -9px;">
      ...
      <div class="container-fluid">
         <div class="row">
            ...
            <div id="footer" class="clearfix">
                <span class="pull-left"><?php echo lang("ctn_170") ?> <a href="http://www.devfy.com/">Devfy</a> <?php echo $this->settings->info->site_name ?> V<?php echo $this->settings->version ?></span> <span class="pull-right"><a href="<?php echo site_url("account/change_language") ?>"><?php echo lang("ctn_171") ?></a></span>
            </div>
        </div>
    </div>
    <script type="text/javascript">
        $(document).ready(function() {
            $('select[name="pais"]').on('change', function() {
                ...
                if(stateID) {
                    $.ajax({
                        url: '<?php echo base_url(); ?>property/estados/'+stateID,
                        type: "GET",
                        dataType: "json",
                        success: function(data) {
                            $('select[name="estado"]').empty();
                            $.each(data, function(key, value) {
                                $('select[name="estado"]').append('<option value="'+ value.id +'">'+ value.estadonombre +'</option>');
                            });
                        }
                    });
                }else{
            ...
            $('select[name="provincia"]').on('change', function() {
                ...
                if(stateID) {
                    $.ajax({
                        url: '<?php echo base_url(); ?>property/distritos/'+distritoID,
                        type: "GET",
                        dataType: "json",
                        success: function(data) {
                            $('select[name="distritos"]').empty();
                            $.each(data, function(key, value) {
                                $('select[name="distrito"]').append('<option value="'+ value.id_distritos +'">'+ value.nombre +'</option>');
                            });
                        }
                    });
                }else{
